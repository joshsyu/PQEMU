...
TCGv_i32 cpu_exclusive_addr;
TCGv_i32 cpu_exclusive_hold_lock;
#else
...
#ifdef CONFIG_CPUTHREAD
    cpu_exclusive_hold_lock = tcg_global_mem_new_i32(TCG_AREG0,
        offsetof(CPUState, exclusive_hold_lock), "exclusive_hold_lock");
#endif
#ifdef CONFIG_USER_ONLY
                               ...
                               TCGv addr, int size)
{
    ...
    gen_helper_profiling_start(cpu_env);
#endif
    tcg_gen_lock();
    // __atomic_spin lock
#ifdef CONFIG_RDTSC_COUNT
    gen_helper_profiling_wait(cpu_env);
#endif
    tcg_gen_movi_i32(cpu_exclusive_hold_lock, 1);
#endif
...
#ifdef CONFIG_CPUTHREAD
    // __atomic_spin unlock
    tcg_gen_unlock();
    tcg_gen_movi_i32(cpu_exclusive_hold_lock, 0);
#ifdef CONFIG_RDTSC_COUNT
    gen_helper_profiling_end(cpu_env);
#endif
#endif    
}
...
#ifdef CONFIG_CPUTHREAD
    gen_helper_update_global_monitor(cpu_env);
#endif
...
#ifdef CONFIG_CPUTHREAD
			gen_helper_update_global_monitor_with_addr(addr);
#endif
                        dead_tmp(addr);
                        store_reg(s, rd, tmp2);
#ifdef CONFIG_CPUTHREAD
